 ...
 *
 * On alloue explicitement via `new ArrayBuffer(len)` puis `new Uint8Array(buf)`
 * pour que TypeScript infère `Uint8Array<ArrayBuffer>` plutôt que
 * `Uint8Array<ArrayBufferLike>` (qui inclut SharedArrayBuffer et n'est pas
 * accepté par BufferSource côté DOM lib en TS strict). */
