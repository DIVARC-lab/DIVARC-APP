...
export async function POST(request: Request) {
  ...
  /* Rate limit basique : max 10 analyses/heure par user. Le coût étant
     ~0.05€/analyse, on évite l'abus + le DDoS via le mécanisme cache. */
  ...
  if ((recentCount ?? 0) >= 10) {
    return NextResponse.json(
      {
        error:
          "Limite atteinte (10 analyses/heure). Réessaie plus tard ou contacte le support.",
      },
      { status: 429 },
    );
  }
  ...
  if (result.status === "failed") {
    return NextResponse.json(
      ...
      { status: 502 }, // 502 plutôt que 500 — c'est l'analyse qui a échoué (souvent côté site externe)
    );
  }
